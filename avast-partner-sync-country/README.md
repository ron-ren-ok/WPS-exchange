...
以附件 CSV 中四个指定 campaign 最新的有数据日期为锚点，同步该日及其前两天（例如最新为 8 月 23 日，则同步 8 月 21–23 日）。CSV 的 `installs` 写入新增；血量按「合作方价格」中 Avast 的国家代码、运营位价格计算（`新增 × 价格`）。缺少价格时仍写新增，血量留空（已有血量不改）。已有键的新增和血量会用计算结果覆盖。
