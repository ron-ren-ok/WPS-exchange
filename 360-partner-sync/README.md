...
源表未提供血量，因此同步器只写入或更新「新增」，绝不覆盖目标长表中已有的「血量」。记录以「日期 + 合作方(360) + 运营位」定位，不存在则追加。日常运行仅从各运营位最后有效日期向后同步，并检查最近 14 天的空白新增；不会自动回补早于该窗口的历史空洞。仅手动传入 `start_date` 时才会执行历史补数。已有的非空且不一致数据会保留，并在日志中列为 `skipped_conflicts`；源表为空的日期不会写入 0，而会在 `unavailable_records` 中报告，且不阻塞其他运营位。
